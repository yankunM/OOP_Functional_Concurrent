...
public class HigherOrderUtils {
    ...
    /**
     * Interface with one additional method declaration name().
     * This Interface extends java.util.function.BiFunction.
     *
    ...
    public static <T> T zip(List<T> args, List<? extends BiFunction<T, T, T>> bifunctions){
        ...
        for (int i = 0; i < bifunctions.size(); i++) {
            T r = bifunctions.get(i).apply(args.get(i), args.get(i + 1));
            if (r == null) {
                return null;
            } else {
                args.set(i + 1, r);
